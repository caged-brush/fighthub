...
export default function fightClipsRoutes(supabase, supabaseAdmin, requireAuth) {
    ...
    router.post("/create", requireAuth, async (req, res) => {
        const userId = String(req.user.id);
        const { fight_date = null, opponent = null, promotion = null, result = "win", weight_class = null, notes = null, storage_path, mime_type, file_size, visibility = "public", } = req.body;
        if (!storage_path) {
    ...
    router.post("/create-youtube", requireAuth, async (req, res) => {
        const userId = String(req.user.id);
        const { fight_date = null, opponent = null, promotion = null, result = "win", weight_class = null, notes = null, youtube_url, visibility = "public", } = req.body;
        if (!youtube_url) {
    ...
    router.get("/feed", requireAuth, async (req, res) => {
      ...
      `)
            ...
            if (error)
                throw error;
            const nextCursor = clips && clips.length ? clips[clips.length - 1].created_at : null;
            return res.json({
                clips: clips || [],
                nextCursor,
            });
