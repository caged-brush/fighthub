...
async function getGymManagementAccess(gymId, userId) {
...
router.post("/gyms", requireAuth, async (req, res) => {
    try {
        ...
        const { name, bio, city, region, country = "Canada", website, instagram, logo_path, } = req.body;
...
router.get("/gyms/:id", requireAuth, async (req, res) => {
    try {
...
router.patch("/gyms/:id", requireAuth, async (req, res) => {
    try {
        const gymId = req.params.id;
        if (!req.user) {
            return res.status(401).json({ message: "Unauthorized" });
...
router.get("/gyms/:id/requests", requireAuth, async (req, res) => {
    try {
        const gymId = req.params.id;
        if (!req.user) {
            return res.status(401).json({ message: "Unauthorized" });
...
router.get("/gyms/:id/roster", requireAuth, async (req, res) => {
    try {
        const gymId = req.params.id;
        if (!req.user) {
            return res.status(401).json({ message: "Unauthorized" });
...
router.post("/gyms/:gymId/memberships/:membershipId/approve", requireAuth, async (req, res) => {
    try {
        ...
        console.error("POST /coach/gyms/:gymId/memberships/:membershipId/approve error:", err);
        return res.status(500).json({
            message: err?.message || "Internal server error.",
        });
    }
});
router.post("/gyms/:gymId/memberships/:membershipId/reject", requireAuth, async (req, res) => {
    try {
        const { gymId, membershipId } = req.params;
        if (!req.user) {
            return res.status(401).json({ message: "Unauthorized" });
        ...
        console.error("POST /coach/gyms/:gymId/memberships/:membershipId/reject error:", err);
        return res.status(500).json({
            message: err?.message || "Internal server error.",
        });
    }
});
router.delete("/gyms/:gymId/memberships/:membershipId", requireAuth, async (req, res) => {
    try {
        const { gymId, membershipId } = req.params;
        if (!req.user) {
            return res.status(401).json({ message: "Unauthorized" });
        ...
        console.error("DELETE /coach/gyms/:gymId/memberships/:membershipId error:", err);
        return res.status(500).json({
            message: err?.message || "Internal server error.",
        });
    }
