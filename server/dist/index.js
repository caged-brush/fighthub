...
function validateUserInput(req, res, next) {
    ...
    const validPassword = (pwd) => /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/.test(pwd);
    if (req.path === "/register") {
        ...
        if (!validPassword(password))
            return res.status(401).json({
                error: "Password must be at least 8 characters long and include uppercase, lowercase, digit, and special char",
            });
        if (password !== confirm)
...
console.log("[SUPABASE ENV]", {
    ...
    sameKey: (process.env.SUPABASE_ANON_KEY || "").trim() ===
