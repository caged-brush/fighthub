...
if (!supabaseAdmin) {
    console.error("Missing SUPABASE_SERVICE_ROLE_KEY. Set it in server/.env and restart.");
    process.exit(1);
}
        ...
        if (pErr) {
            ...
            for (const post of posts) {
                const { error: upErr } = await supabaseDefault
                    ...
                    .eq("id", post.id);
                if (upErr)
                    console.warn("posts update error:", upErr);
                else
                    console.log(`updated post ${post.id} media_url -> ${storagePath}`);
            }
        }
        ...
        if (uErr) {
            ...
            for (const u of users) {
                const { error: upErr } = await supabaseDefault
                    ...
                    .eq("id", u.id);
                if (upErr)
                    console.warn("users update error:", upErr);
                else
                    console.log(`updated user ${u.id} profile_picture_url -> ${storagePath}`);
            }
        }
    }
    catch (err) {
