...
export default function setupSocket(io, supabase) {
    ...
    io.use((socket, next) => {
        ...
        try {
            ...
            console.log("[socket auth] decoded keys:", decoded ? Object.keys(decoded) : null);
            console.log("[socket auth] decoded exp:", decoded?.exp);
            const payload = jwt.verify(token, process.env.JWT_SECRET);
            const uid = String(payload?.id || payload?.userId || payload?.sub || "").trim();
            if (!uid)
                return next(new Error("unauthorized:missing_uid_claim"));
        ...
        socket.on("load-messages", async ({ recipientId }) => {
            ...
            try {
                ...
                const { data, error } = await supabase
                    ...
                    .or(`and(sender_id.eq.${uid},recipient_id.eq.${rid}),and(sender_id.eq.${rid},recipient_id.eq.${uid})`)
